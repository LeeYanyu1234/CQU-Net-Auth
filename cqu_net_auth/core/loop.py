    ...
    while True:
        ...
        if "uid" in auth_info and auth_info["uid"] != config.account:
            try:
                if portal_client.logout(auth_info["uid"], auth_info["v46ip"]):
                    logger.info("portal.previous_account_logged_out: %s", auth_info["uid"])
                    status = "unauth"
                    continue
            except PortalClientError as exc:
                logger.warning("portal.logout_failed_invalid_input: %s", exc)
            logger.error("portal.failed_to_logout_previous_account: %s", auth_info["uid"])
            status = "uncertain"
            time.sleep(config.interval)
            continue
        ...
        if "uid" in auth_info:
            ...
            if portal_ip and last_portal_ip and portal_ip != last_portal_ip:
                notifier.notify_portal_ip_changed(config.account, last_portal_ip, portal_ip)

            if portal_ip:
                if portal_ip != last_portal_ip:
                    record_ip_to_file(config.file_path, uid=auth_info.get("uid"), portal_ip=portal_ip)
        ...
        result, msg = portal_client.login(config.account, config.password, config.term_type, portal_ip)
        if not result:
            status = "unauth"
            if msg in ["账号不存在", "密码错误"]:
                logger.error("portal.auth_failed_fatal: account=%s term_type=%s msg=%s", config.account, config.term_type, msg)
                sys.exit(-1)
            if "等待5分钟" in msg:
                ...
                logger.warning("portal.auth_failed_retry: account=%s term_type=%s msg=%s", config.account, config.term_type, msg)
                time.sleep(config.interval)
        else:
            status = "auth"
            logger.info("portal.auth_success: account=%s term_type=%s", config.account, config.term_type)
            last_portal_ip = portal_ip
            record_ip_to_file(config.file_path, uid=config.account, portal_ip=portal_ip)
